...
import { adaptQuiz, adaptLeaderboardEntry } from '../../../lib/adapters';
...
export default function QuizListingPage() {
  ...
  const fetchQuizzes = useCallback(() => {
    ...
    quizzesService.getQuizzes({
      ...
      difficulty: selectedDifficulty ? selectedDifficulty.toUpperCase() : undefined,
      limit: 50,
    })
  ...
  useEffect(() => {
    usersService.getLeaderboard({ limit: 10 })
      .then((res) => setLeaderboard((res.data.data || []).map(adaptLeaderboardEntry)))
      .catch(() => setLeaderboard([]));
  }, [period]);

  const topThree = leaderboard.slice(0, 3);
  const restOfUsers = leaderboard.slice(3, 10);
      ...
      >
        ...
        <div className="mb-8 p-6 bg-white rounded-xl shadow-sm">
          ...
          <div className="flex gap-2">
            <Button variant={selectedDifficulty === '' ? 'default' : 'outline'} size="sm" onClick={() => setSelectedDifficulty('')}>Tất Cả</Button>
            <Button variant={selectedDifficulty === 'easy' ? 'default' : 'outline'} size="sm" onClick={() => setSelectedDifficulty('easy')}>Dễ</Button>
            <Button variant={selectedDifficulty === 'medium' ? 'default' : 'outline'} size="sm" onClick={() => setSelectedDifficulty('medium')}>Trung Bình</Button>
            <Button variant={selectedDifficulty === 'hard' ? 'default' : 'outline'} size="sm" onClick={() => setSelectedDifficulty('hard')}>Khó</Button>
          </div>
        </div>

        {/* Results Count */}
        <div className="mb-6">
          <p className="text-gray-600">Có {quizzes.length} quiz</p>
        </div>
        ...
        <div className="mt-16">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">
              {language === 'vi' ? 'Học viên hóa học xuất sắc nhất cộng đồng' : 'Top chemistry learners in the community'}
            </p>
          </div>
          ...
          <div className="flex justify-center gap-2 mb-12">
            ...
            <Button variant={period === 'all-time' ? 'default' : 'outline'} onClick={() => setPeriod('all-time')}>
                  ...
                  >
                    <Link to={`/users/${user.id}`} className="block">
                      <div className="relative mb-4">
                        <img src={user.avatar} alt={user.name} className="w-24 h-24 rounded-full border-4 border-white shadow-lg hover:border-blue-300 transition-colors cursor-pointer" />
                      ...
                      <div className={`bg-white rounded-t-2xl shadow-xl p-6 w-48 text-center ${getPodiumHeight(user.rank)} flex flex-col justify-between hover:shadow-2xl transition-shadow cursor-pointer`}>
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{language === 'vi' ? 'điểm' : 'points'}</p>
                        </div>
                        <div className="grid grid-cols-2 gap-3 text-sm mt-2">
                          ...
                          <div className="text-center">
                            ...
                            <div className="text-xs text-gray-500 truncate">{language === 'vi' ? 'Câu hỏi' : 'Questions'}</div>
                          </div>
                        </div>
                      </div>
                    </Link>
                    ...
                    <Link to={`/users/${user.id}`} className="flex items-center gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 text-sm text-gray-600">
                          ...
                          <span>•</span>
                          <span>{user.questionsPosted} {language === 'vi' ? 'câu hỏi' : 'questions'}</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-2xl font-bold text-purple-600 mb-1">{user.points.toLocaleString()}</div>
                        <div className="text-sm text-gray-500">{language === 'vi' ? 'điểm' : 'points'}</div>
                      </div>
                    </Link>
                  </motion.div>
                ))}
